...
func RegisterPermissionRoutes(r *mux.Router, secretKey string) {
	...
	permissions.Handle("/", middleware.PermissionMiddleware(database.DB, "permission:read")(http.HandlerFunc(handlers.CheckHealth))).Methods(http.MethodGet)
	permissions.Handle("/{permission_id}", middleware.PermissionMiddleware(database.DB, "permission:read")(http.HandlerFunc(handlers.CheckHealth))).Methods(http.MethodGet)
}